...
def sendReadingsToMqtt(datatimestamp, co2, temp, humidity):
  payload = '{{ "time": "{}", "co2": {}, "temperature": {}, "humidity": {} }}'.format(datatimestamp, co2, temp, humidity)
  client = MQTTClient(MQTT_CONFIG.MQTT_CLIENT_ID, MQTT_CONFIG.MQTT_BROKER)
  client.connect()
  print("Connected to MQTT Broker")
  client.publish(MQTT_CONFIG.MQTT_TOPIC, payload, True)
  print("Published")
  client.disconnect()
...
def drawScreen(timestamp, co2, temperature, humidity):
  ...
  badger.set_thickness(1)
  badger.text(timestamp, 152, 22, False, 0.4)
...
while True:
  if breakout_scd41.ready():
    ...
    try:
      ...
      ntptime.settime()
      ts = time.gmtime()
      datatimestamp = '{}-{:02}-{:02}T{:02}:{:02}:{:02}Z'.format(*ts)
      timestamp = '{:02}:{:02}'.format(ts[3], ts[4])
      sendReadingsToMqtt(datatimestamp, co2, temperature, humidity)
      wifi_disconnect()
    except:
      print("MQTT publish failed")
      
    print("Refreshing screen")
    drawScreen(timestamp, co2, temperature, humidity)
